var API_BASE = 'https://ydfvfdizipanel.ru/public/api';
var API_KEY = '9iQNC5HQwPlaFuJDkhncJ5XTJ8feGXOJatAA';

// SineWix'in (EasyPlex) orijinal header yapısı
var API_HEADERS = {
    'hash256': '711bff4afeb47f07ab08a0b07e85d3835e739295e8a6361db77eebd93d96306b',
    'signature': '',
    'User-Agent': 'EasyPlex (Android 14; SM-A546B; Samsung Galaxy A54 5G; tr)',
    'Accept': 'application/json'
};

async function getStreams(id, mediaType, seasonNum, episodeNum) {
    try {
        const tmdbId = id.toString().split(':')[0];
        const tmdbRes = await fetch(`https://api.themoviedb.org/3/${mediaType === 'movie' ? 'movie' : 'tv'}/${tmdbId}?api_key=4ef0d7355d9ffb5151e987764708ce96&language=tr-TR`);
        const tmdb = await tmdbRes.json();
        
        const query = (tmdb.name || tmdb.title || "").toLowerCase();
        const searchRes = await fetch(`${API_BASE}/search/${encodeURIComponent(query)}/${API_KEY}`, { headers: API_HEADERS });
        const searchData = await searchRes.json();
        
        const list = searchData.search || [];
        if (list.length === 0) return [];

        // Euphoria gibi diziler için en iyi eşleşmeyi bul
        const match = list.find(i => (i.name||"").toLowerCase().includes(query) && (mediaType === 'movie' ? i.type !== 'serie' : i.type === 'serie')) || list[0];

        const path = (mediaType === 'movie') ? `media/detail/${match.id}` : `series/show/${match.id}`;
        const detailRes = await fetch(`${API_BASE}/${path}/${API_KEY}`, { headers: API_HEADERS });
        const item = await detailRes.json();

        let vLinks = [];
        if (mediaType === 'movie') {
            vLinks = item.videos || [];
        } else {
            const s = (item.seasons || []).find(x => parseInt(x.season_number) === parseInt(seasonNum));
            const e = s ? (s.episodes || []).find(x => parseInt(x.episode_number) === parseInt(episodeNum)) : null;
            vLinks = e ? (e.videos || []) : [];
        }

        return vLinks.map((v, idx) => {
            const rawUrl = (v.link || "").trim();
            
            // KRİTİK: Eğer link m3u8 veya mp4 değilse, player hata verebilir.
            // Ama biz yine de SineWix'in beklediği Referer ile gönderiyoruz.
            return {
                name: "SineWix",
                title: `⌜ RECTV ⌟ | ${v.server || 'Kaynak ' + (idx + 1)}`,
                url: rawUrl,
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36',
                    'Referer': 'https://ydfvfdizipanel.ru/', // SineWix'in kendi paneli üzerinden gelmiş gibi göster
                    'Accept-Encoding': 'identity'
                }
            };
        }).filter(res => res.url.startsWith('http'));

    } catch (e) {
        return [];
    }
}

module.exports = { getStreams };
