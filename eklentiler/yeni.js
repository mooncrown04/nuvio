...
// SABİT HEADER VE SIGNATURE BLOĞU - DOKUNULMADI
var API_HEADERS = {
...
function searchAndFetch(title, originalTitle, targetImdb, mediaType, seasonNum, episodeNum) {
    ...
    return fetch(searchUrl, { headers: API_HEADERS })
        ...
        .then(function(data) {
            ...
            return Promise.all(allResults.map(resItem => {
                ...
                return fetch(API_BASE + '/' + path + '/' + resItem.id + '/' + API_KEY, { headers: API_HEADERS })
                    .then(r => r.json())
                    .catch(err => {
                        console.error("SineWix Detay Hatasi (ID: " + resItem.id + "): " + err.message);
                        return null;
                    });
            })).then(function(detailedItems) {
                var bestMatch = detailedItems.find(item => item && item.imdb_external_id === targetImdb);
                
                if (!bestMatch) {
                    bestMatch = detailedItems.find(item => item && item.name.toLowerCase().includes(originalTitle.toLowerCase()));
                }
        ...
        .catch(function(error) {
            // Sadece hata durumunda yazar
            console.error("SineWix Arama Islemi Basarisiz: " + error.message);
            return [];
        });
}
...
function getStreams(id, mediaType, seasonNum, episodeNum) {
    return new Promise(function(resolve) {
        ...
        fetch(tmdbUrl).then(res => res.json()).then(function(data) {
            ...
            console.error("Sistem Genel Hatasi: " + err.message);
            resolve([]);
        });
    });
}
