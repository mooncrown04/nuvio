...
function fetchDetailAndStreams(sinewixId, mediaType, seasonNum, episodeNum) {
    ...
    return fetch(apiUrl, { headers: API_HEADERS })
        ...
        .then(function(item) {
            ...
            console.error('[LINK_SAYISI]: ' + vLinks.length);
...
function searchAndFetch(title, originalTitle, mediaType, seasonNum, episodeNum) {
    ...
    console.error('[ARAMA_URL_TR]: ' + searchUrl);
    ...
    return fetch(searchUrl, { headers: API_HEADERS })
        ...
        .then(function(data) {
            ...
            // EGER TURKCE ISIMLE SONUC YOKSA, ORIJINAL ISMI DENE
            if (results.length === 0 && originalTitle && originalTitle !== title) {
                console.error('[BILGI]: Turkce bos dondu, Orijinal Isim deneniyor: ' + originalTitle);
                var originalUrl = API_BASE + '/search/' + encodeURIComponent(originalTitle) + '/' + API_KEY;
                return fetch(originalUrl, { headers: API_HEADERS }).then(function(r) { return r.json(); });
            }
            return data;
        })
        .then(function(data) {
            ...
            if (filtered.length === 0) { console.error('[HATA]: İki isimle de sonuç yok!'); return []; }

            // Tam isim eslesmesi kontrolü
            var best = filtered.find(function(i) {
                var n = (i.name || '').toLowerCase().trim();
                return n === title.toLowerCase().trim() || n === (originalTitle || '').toLowerCase().trim();
            }) || filtered[0];
    ...
    return new Promise(function(resolve) {
        ...
        fetch(tmdbUrl)
            ...
            .then(function(data) {
                ...
                return searchAndFetch(title, originalTitle, mediaType, sNum, eNum);
            })
