//v1
var API_BASE = 'https://ydfvfdizipanel.ru/public/api';
var API_KEY = '9iQNC5HQwPlaFuJDkhncJ5XTJ8feGXOJatAA';

var API_HEADERS = {
    'hash256': '711bff4afeb47f07ab08a0b07e85d3835e739295e8a6361db77eebd93d96306b',
    "signature": "",
    'User-Agent': 'EasyPlex (Android 14; SM-A546B; Samsung Galaxy A54 5G; tr)',
    'Accept': 'application/json'
};

function fetchDetailAndStreams(sinewixId, mediaType, seasonNum, episodeNum) {
    var genre = (mediaType === 'movie') ? 'media' : 'series';
    var endpoint = (mediaType === 'movie') ? 'detail' : 'show';
    var apiUrl = API_BASE + '/' + genre + '/' + endpoint + '/' + sinewixId + '/' + API_KEY;
    
    // NOKTA ATISI 1: Detay URL'sini kontrol et
    console.error('[DETAY_URL]: ' + apiUrl);

    return fetch(apiUrl, { headers: API_HEADERS })
        .then(function(res) { return res.json(); })
        .then(function(item) {
            var title = item.name || item.title || 'SineWix';
            var year = (item.first_air_date || item.release_date || '').substring(0, 4);
            var videoLinks = [];

            if (mediaType === 'movie') {
                videoLinks = (item.videos || []).map(function(v) { return v.link; }).filter(Boolean);
            } else {
                var targetSeason = (item.seasons || []).find(function(s) { return parseInt(s.season_number) === parseInt(seasonNum); });
                if (targetSeason) {
                    var targetEp = (targetSeason.episodes || []).find(function(e) { return parseInt(e.episode_number) === parseInt(episodeNum); });
                    if (targetEp) {
                        videoLinks = (targetEp.videos || []).map(function(v) { return v.link; }).filter(Boolean);
                    } else {
                        console.error('[HATA]: Bolum ' + episodeNum + ' bulunamadi!');
                    }
                } else {
                    console.error('[HATA]: Sezon ' + seasonNum + ' bulunamadi!');
                }
            }

            // NOKTA ATISI 2: Link sayısını kontrol et
            console.error('[LINK_SAYISI]: ' + videoLinks.length);

            return videoLinks.map(function(link) {
                return {
                    name: 'SineWix',
                    title: title + (year ? ' (' + year + ')' : ''),
                    url: link,
                    headers: { 'Referer': 'https://ydfvfdizipanel.ru/' }
                };
            });
        }).catch(function(e) { console.error('[DETAY_HATASI]: ' + e.message); return []; });
}

function searchAndFetch(title, mediaType, seasonNum, episodeNum) {
    var searchUrl = API_BASE + '/search/' + encodeURIComponent(title) + '/' + API_KEY;
    console.error('[ARAMA_URL]: ' + searchUrl);

    return fetch(searchUrl, { headers: API_HEADERS })
        .then(function(res) { return res.json(); })
        .then(function(data) {
            var results = data.search || [];
            var filtered = results.filter(function(item) {
                var t = item.type || '';
                return (mediaType === 'movie') ? t.includes('movie') : (t.includes('serie') || t.includes('anime'));
            });

            if (filtered.length === 0) { console.error('[HATA]: SineWix aramada hic sonuc yok!'); return []; }

            // NOKTA ATISI 3: "From" gibi kisa isimlerde tam eslesmeyi oncelik yap
            var best = filtered.find(function(i) {
                return (i.name || '').toLowerCase().trim() === title.toLowerCase().trim();
            }) || filtered[0];

            console.error('[SECILEN]: ' + best.name + ' | ID: ' + best.id);
            return fetchDetailAndStreams(best.id, mediaType, seasonNum, episodeNum);
        }).catch(function(e) { console.error('[ARAMA_HATASI]: ' + e.message); return []; });
}

function getStreams(id, mediaType, seasonNum, episodeNum) {
    return new Promise(function(resolve) {
        // tt...:1:1 formatini parcala
        var parts = id.toString().split(':');
        var tmdbId = parts[0];
        var sNum = parts[1] || seasonNum || 1;
        var eNum = parts[2] || episodeNum || 1;

        var tmdbUrl = 'https://api.themoviedb.org/3/' + (mediaType === 'movie' ? 'movie' : 'tv') + '/' + tmdbId + '?language=tr-TR&api_key=4ef0d7355d9ffb5151e987764708ce96';

        fetch(tmdbUrl)
            .then(function(res) { return res.json(); })
            .then(function(data) {
                var title = data.name || data.title;
                if (!title) { console.error('[TMDB_HATA]: Isim alinamadi!'); return resolve([]); }
                return searchAndFetch(title, mediaType, sNum, eNum);
            })
            .then(function(streams) { resolve(streams || []); })
            .catch(function(e) { console.error('[GENEL_HATA]: ' + e.message); resolve([]); });
    });
}

module.exports = { getStreams };
