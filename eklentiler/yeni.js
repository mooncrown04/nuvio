/**
 * SineWix_Fix_v3
 * FİLM & DİZİ: Aranan isimden çok uzun olan (ek almış) sonuçları eler.
 * SIRALAMA: İsmi aranan isme en yakın (en kısa) olanı otomatik ilk sıraya alır.
 */
...
function searchAndFetch(title, mediaType, seasonNum, episodeNum) {
    ...
    return fetch(searchUrl, { headers: API_HEADERS })
        ...
        .then(function(data) {
            ...
            // --- GELİŞMİŞ FİLTRELEME (Hem Dizi Hem Film) ---
            var filtered = results.filter(function(item) {
                ...
                // 2. "From" gibi kısa isimler için tam eşleşme kontrolü
                if (sTitleLower === "from" || sTitleLower.length <= 4) {
                    return itemTitleLower === sTitleLower;
                }

                // 3. Uzunluk Kontrolü (Aranan isimden çok uzun olanları eler, örn: "Yol" ararken "Yollar" gelmez)
                // +4 pay bırakıyoruz (bazı ekler veya parantez içindeki yıllar için)
                var lengthOk = itemTitleLower.length <= sTitleLower.length + 4;
            ...
            // 4. SIRALAMA: Aranan isme en yakın uzunlukta olanı en başa al
