...
function searchAndFetch(title, originalTitle, mediaType, seasonNum, episodeNum) {
    ...
    return fetch(searchUrl, { headers: API_HEADERS })
        ...
        .then(function(data) {
            var res = data.search || [];
            if (res.length === 0) { console.error('[HATA]: Arama sonucsuz kaldı.'); return []; }

            // 3. AŞAMA: AKILLI SEÇİM (Tam Eşleşme > Puanlama)
            var best = res.find(function(i) {
                var name = (i.name || '').toLowerCase().trim();
                return name === title.toLowerCase().trim() || name === (originalTitle || '').toLowerCase().trim();
            });

            if (!best) {
                // Tam eşleşme yoksa puanı (rate) en yüksek olanı al
                best = res.sort(function(a, b) { return parseFloat(b.rate || 0) - parseFloat(a.rate || 0); })[0];
            }

            console.error('[ANALIZ]: Aranan: ' + title + ' | Secilen: ' + best.name + ' | ID: ' + best.id + ' | Puan: ' + best.rate);
