/**
 * FullHDFilmizlesene Nuvio Scraper - v28.2 (Stabilize & Debug)
 * Geliştirici: moOncrOwN
 */
...
async function getStreamsFromAPI(vidid, movieTitle) {
    const fetchAtom = async () => {
        ...
        } catch (e) { console.error("FullHD-Error: Atom API Hatası - " + vidid); }
        return null;
    };
    ...
    const fetchTurbo = async () => {
        ...
        } catch (e) { console.error("FullHD-Error: Turbo API Hatası - " + vidid); }
        return null;
    };
...
function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
    return new Promise(function(resolve) {
        ...
        fetch('https://api.themoviedb.org/3/movie/' + tmdbId + '?language=tr-TR&api_key=4ef0d7355d9ffb5151e987764708ce96')
            .then(res => res.json())
            .then(data => {
                if (!data.id) throw new Error("TMDB Verisi Yok");
                const year = data.release_date ? data.release_date.split('-')[0] : "";
                const movieTitle = data.title || data.original_title;
                const ttId = data.imdb_id;
                
                // IMDb ID varsa öncelikli arama, yoksa isimle arama[cite: 1]
            ...
            .then(async ([res, year, movieTitle]) => {
                ...
                // Genişletilmiş Seçiciler: .list li (arama sayfası) ve .film-listesi li (kategori sayfası)[cite: 1]
                $("ul.list li, .film-listesi li, .search-results li").each((i, el) => {
                    let link = $(el).find("a.tt").attr("href") || $(el).find("a[href*='/film/']").attr("href");
                    if (link) {
                        let rowText = $(el).text().toLowerCase();
                        if (year === "" || rowText.includes(year.toString())) {
                            filmLink = link;
                            return false; 
                        }
                    }
                ...
                if (!filmLink) filmLink = $("a[href*='/film/']").first().attr("href");

                if (!filmLink) {
                    console.error("FullHD-Error: Film Sayfası Linki Bulunamadı - " + movieTitle);
                    throw new Error("Film bulunamadı");
                }
                
                let finalUrl = filmLink.startsWith('http') ? filmLink : BASE_URL + (filmLink.startsWith('/') ? '' : '/') + filmLink;
                ...
                console.error("FullHD-Error: vidid Bulunamadı - " + finalUrl);
                return [];
            })
