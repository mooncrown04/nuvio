/**
 * DiziPal 1543 - HTML Debugger Version
 */
...
function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
    ...
    return fetch("https://api.themoviedb.org/3/" + type + "/" + tmdbId + "?language=tr-TR&api_key=4ef0d7355d9ffb5151e987764708ce96")
        ...
        .then(function(tmdb) {
            ...
            return fetch(target).then(function(res) { return res.text(); }).then(function(html) { return { html: html, url: target }; });
        })
        .then(function(data) {
            var m = data.html.match(/<div[^>]*data-rm-k="true"[^>]*>(.*?)<\/div>/);
            if (!m) return [];
            var iframe = decryptData(m[1]).replace(/[\\"]/g, "").trim();
            ...
            return fetch(iframe, { headers: { 'Referer': data.url } })
                ...
                .then(function(p) {
                    // --- DEBUG BAŞLIYOR ---
                    // HTML içindeki script bloklarını bulup ilk 500 karakterini logla
                    var scripts = p.match(/<script[^>]*>([\s\S]*?)<\/script>/g);
                    if (scripts) {
                        scripts.forEach(function(s, i) {
                            if (s.length > 50) { // Kısa scriptleri atla
                                console.error("[DiziPal] DEBUG SCRIPT " + i + ": " + s.substring(0, 300).replace(/\n/g, " "));
                            }
                        });
                    }
                    // --- DEBUG BİTTİ ---
                    ...
                    // Mevcut deneme
                    var pidMatch = p.match(/openPlayer\s*\(\s*['"]([^'"]+)['"]/) || p.match(/v=([^"&']+)/);
                    if (pidMatch) {
                        var origin = iframe.split('/').slice(0, 3).join('/');
                        return fetch(origin + "/source2.php?v=" + pidMatch[1], { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                            .then(function(r) { return r.json(); })
                            .then(function(j) {
                                if (j.file) {
                                    console.error("[DiziPal] STREAM FOUND: " + j.file);
                                    return [{ name: "DiziPal", url: j.file.replace(/\\/g, "").replace("m.php", "master.m3u8"), type: 'm3u8' }];
                                }
                                return [];
                            });
                    }
        ...
        .catch(function(e) { console.error("[DiziPal] Hata: " + e.message); return []; });
}
