/**
 * DiziPal 1543 - ES5 Compatibility Fix
 */
...
globalThis.getStreams = getStreams; = "3hPn4uCjTVtfYWcjIcoJQ4cL1WWk1qxXI39egLYOmNv6IblA7eKJz68uU3eLzux1biZLCms0quEjTYniGv5z1JcKbNIsDQFSeIZOBZJz4is6pD7UyWDggWWzTLBQbHcQFpBQdClnuQaMNUHtLHTpzCvZy33p6I7wFBvL4fnXBYH84aUIyWGTRvM2G5cfoNf4705tO2kv";
...
function decryptData(rawContent) {
    try {
        ...
        var result = decrypted.toString(CryptoJS.enc.Utf8) || decrypted.toString(CryptoJS.enc.Latin1);
...
function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
    ...
    return fetch("https://api.themoviedb.org/3/" + type + "/" + tmdbId + "?language=tr-TR&api_key=4ef0d7355d9ffb5151e987764708ce96")
        ...
        .then(function(searchHtml) {
            var regex = new RegExp('href="\/([^"]*' + (mediaType === 'tv' ? 'series|dizi' : 'movie|film') + '[^"]*)"', 'i');
            var pathMatch = searchHtml.match(regex);
            var slug = pathMatch ? pathMatch[1].split('/').pop() : "";
            var targetUrl = BASE_URL + "/" + (mediaType === 'tv' ? 'bolum' : 'film') + "/" + slug;
            if (mediaType === 'tv') targetUrl += "-" + seasonNum + "x" + episodeNum;

            console.error("[DiziPal] Hedef: " + targetUrl);
            return fetch(targetUrl).then(function(r) { return r.text(); }).then(function(html) {
                return { html: html, url: targetUrl };
            });
        })
        .then(function(data) {
            var divMatch = data.html.match(/<div[^>]*data-rm-k="true"[^>]*>(.*?)<\/div>/);
            if (!divMatch) return [];
            var iframeUrl = decryptData(divMatch[1]);
            if (!iframeUrl) return [];
            if (iframeUrl.indexOf("//") === 0) iframeUrl = "https:" + iframeUrl;
            ...
            return fetch(iframeUrl, { headers: { 'Referer': data.url } })
                .then(function(r) { return r.text(); })
                .then(function(playerHtml) {
                    ...
                    return fetch(origin + "/source2.php?v=" + pid[1], {
                        ...
                        if (!api.file) return [];
                        var stream = api.file.replace(/\\/g, "").replace("m.php", "master.m3u8");
                        console.error("[DiziPal] STREAM: " + stream);
                        return [{
                            name: "DiziPal (DPlayer)",
                            url: stream,
                            type: 'm3u8',
                            headers: { 'Referer': iframeUrl, 'Origin': origin }
                        }];
                    });
                });
        })
